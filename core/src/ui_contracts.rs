...
pub struct UiMarketOrderbook {
    ...
    pub bids: Vec<UiOrderbookLevel>,
    pub asks: Vec<UiOrderbookLevel>,
}
...
pub fn build_ui_market(market: &Market, details: Option<&MarketDetails>) -> UiMarket {
    ...
    let image_url = preferred_media_url(details.and_then(|value| nonempty_string(value.image.as_str())));
    let icon_url = preferred_media_url(details.and_then(|value| nonempty_string(value.icon.as_str())));
...
fn preferred_media_url(value: Option<String>) -> Option<String> {
...
mod tests {
    ...
    fn sample_market_details() -> MarketDetails {
        MarketDetails {
            accepting_order_timestamp: Some("2026-03-21T11:45:00Z".to_string()),
            accepting_orders: true,
            active: true,
            archived: false,
            closed: false,
            condition_id: "0xabc".to_string(),
            description: "A simple BTC up/down market".to_string(),
            enable_order_book: true,
            end_date_iso: "2026-03-21T12:00:00Z".to_string(),
            game_start_time: None,
            image: "https://example.com/btc.png".to_string(),
            icon: "https://example.com/btc-icon.png".to_string(),
            ...
            market_slug: "btc-up-or-down-march-21-805am-et".to_string(),
            created_at: None,
            updated_at: None,
            events: None,
        }
    ...
    fn ui_market_normalizes_common_fields() {
        let details = sample_market_details();
        let ui_market = build_ui_market(&sample_market(), Some(&details));
        assert_eq!(ui_market.condition_id, "0xabc");
        assert_eq!(ui_market.market_slug, "btc-up-or-down-march-21-805am-et");
        assert_eq!(ui_market.title, "Will BTC finish above 100,000 this 15m period?");
        assert_eq!(
            ui_market.image_url.as_deref(),
            Some("https://example.com/btc.png")
        );
        assert_eq!(
            ui_market.icon_url.as_deref(),
            Some("https://example.com/btc-icon.png")
        );
        assert_eq!(ui_market.symbol.as_deref(), Some("BTC"));
        assert_eq!(ui_market.timeframe.as_deref(), None);
        assert_eq!(ui_market.status, "active");
        assert!(ui_market.tradable);
        assert_eq!(ui_market.sides.len(), 2);
        assert_eq!(ui_market.sides[0].label, "Up");
        assert!(ui_market.subtitle.contains("Closes 2026-03-21 12:00:00 UTC"));
    }
}
