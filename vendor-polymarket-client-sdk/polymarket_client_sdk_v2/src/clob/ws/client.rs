...
impl<S: State> Client<S> {
    ...
    /// Close all websocket channel tasks owned by this client.
    pub fn close(&self) {
    ...
    /// Unsubscribe multiple market stream registrations for the same asset set.
    ///
    /// The SDK's market stream APIs all share the same market-channel asset refcount.
    /// Callers that created orderbook, price, and last-trade streams for the same
    /// assets need to decrement the refcount once per stream when replacing a full
    /// subscription set in place.
    pub fn unsubscribe_market_streams(
        &self,
        asset_ids: &[U256],
        stream_count: usize,
    ) -> Result<()> {
