...
The submit path does not reject solely on decision age; readiness is enforced by the pre-tick quote, CEX-depth, DVOL, metadata, direction, price, size, and proxy-freshness gates.
...
10. Derive a dynamic edge-safe limit price from DVOL fair probability, Polymarket taker fees, and the configured edge floor, then floor it to the market tick.
11. Enforce quote freshness, market constraints, entry-price floor, visible-depth VWAP, and fee-aware edge.
12. Apply share sizing and cap checks.
13. Enqueue to arbiter/trader.
14. Enforce submit-time proxy freshness and cached-metadata fast submit checks.
