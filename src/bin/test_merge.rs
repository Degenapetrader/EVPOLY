...
fn run_unit_tests() -> bool {
    ...
    for (i, (up, down, expected)) in cases.iter().enumerate() {
        ...
        println!(
            "       → complete_sets={:.2}, remaining_up={:.2}, remaining_down={:.2}",
            r.complete_sets, r.remaining_up, r.remaining_down
        );
        if !pass {
            println!(
                "       EXPECTED: complete_sets={:.2}, remaining_up={:.2}, remaining_down={:.2}",
                expected.complete_sets, expected.remaining_up, expected.remaining_down
            );
        }
    }
...
async fn main() -> Result<()> {
    ...
    let (condition_id, title) = if let Some(cid) = &args.condition_id {
        (
            cid.clone(),
            format!("Merge check for condition {}...", &cid[..cid.len().min(16)]),
        )
    } else {
    ...
    if args.merge && result.complete_sets > 0.0 {
        println!(
            "🔄 Merging {:.6} complete set(s) (Up+Down → USDC)...",
            result.complete_sets
        );
        match api
            .merge_complete_sets(&condition_id, result.complete_sets)
            .await
        {
            Ok(res) => {
                if res.success {
