...
- `mm_sport_v1` / `endgame_sweep_v1`: fixed deposit-wallet pUSD balance/allowance cache preservation during CLOB `/balance-allowance` rate-limit backoff (`src/api.rs`). Non-forced collateral allowance refresh no-ops and transient update failures no longer clear the cached pUSD snapshot, so MM Sport GTD BUY collateral checks and Endgame collateral prewarm can reuse the intended 180s fallback snapshot during 429/backoff. Real insufficient pUSD balance and insufficient allowance checks still fail through `ensure_buy_collateral_ready`, and the Endgame/EVSnipe fast FAK submit preflight bypass remains unchanged.
- `endgame_sweep_v1` and `evsnipe_v1`: removed the synchronous pUSD collateral/balance-allowance preflight from latency-critical fast FAK BUY submits while leaving it active for resting/manual/MM-style BUY paths (`src/api.rs`, `src/trader.rs`). Endgame cached-metadata FAK submits and EVSnipe fast FAK submits now go straight to CLOB after local strategy gates; if the wallet is actually missing collateral or allowance, CLOB remains the final reject authority and runtime classification still maps those upstream failures to balance/allowance errors.
- `endgame_sweep_v1`: rolled the live Endgame entry model back to the pre-DVOL local V1 three-checkpoint path while keeping the later reliability fixes that were safe for that path (`src/main.rs`, `src/trader.rs`, `src/config.rs`, `src/endgame_sweep.rs`, `src/size_policy.rs`, `src/endgame_cex_depth.rs`, `src/lib.rs`, `.env.example`, `.env.full.example`, `docs/endgame_sweep_v1.md`). Default checkpoints are hardcoded again to `2000,1000,100` ms before close with `20/40/40` share-size allocation, fixed `0.99` FAK BUY submits, and Polymarket mid-band gates; stale `EVPOLY_ENDGAME_TICK_OFFSETS_*` profile values no longer move the legacy checkpoint schedule. Endgame now uses per-checkpoint proxy/base thresholds of `2.0/1.5/1.0` bps, applies the existing RTDS Chainlink guard as a hard skip when ready and as a near-base bps surcharge when not ready, and treats Book99-style CEX-depth as mandatory. Weak CEX-depth can reduce size, strong CEX-depth always can increase size, and the live submit multiplier is clamped to `0.0..2.0` while still capped by remaining period budget. The Endgame worker/trader close guard is `25ms` so the intended `t-100ms` final FAK slot remains reachable. The removed DVOL/ATR fair-price module is no longer part of Endgame.

### 2026-06-20
- `endgame_sweep_v1`: fixed fast FAK BUY fill accounting after live cheaper-than-limit fills were misclassified as no-fill. Endgame now accepts notional-capped FAK BUY responses where filled shares exceed the nominal requested share count because the execution price was below the limit, and runtime positive FAK matches emit canonical `ENTRY_FILL` events so `fills_v2` / `positions_v2` and strategy reports count them immediately (`src/trader.rs`).
- `endgame_sweep_v1`: added a Book99-style RTDS Chainlink guard and ATR hardening to the live DVOL fair-price path while preserving the existing proxy base/current calculation (`src/endgame_rtds.rs`, `src/endgame_dvol.rs`, `src/main.rs`, `.env.example`, `.env.full.example`). When RTDS has the exact period-open sample and a fresh current sample, it hard-skips selected sides whose oracle move disagrees with the trade direction or is below `EVPOLY_ENDGAME_RTDS_GUARD_MIN_BPS`; when RTDS is unavailable it adds `EVPOLY_ENDGAME_RTDS_NO_GUARD_EXTRA_BPS` to the DVOL required distance instead of replacing the proxy base. DVOL required distance now clamps negative raw requirements to zero before RTDS/CEX-depth/ATR multipliers, and ATR can only harden the requirement by default (`EVPOLY_ENDGAME_ATR_MIN_MULT=1.0`). Endgame fast BUY submits now use non-resting `FAK` orders instead of resting `GTC` orders, require visible executable ask depth for share sizing, kill the unfilled remainder, and track only the inferred positive partial fill instead of the requested order size.

### 2026-06-19
- `endgame_sweep_v1`: fixed the live CEX-depth mandatory gate after in-period restarts. When a venue-specific external-depth open-mid sample is missing but the exact Endgame period base anchor is available, CEX-depth now uses that exact strategy anchor as the baseline instead of fail-opening every tick with `base_price_missing` and producing zero submits (`src/endgame_cex_depth.rs`, `docs/endgame_sweep_v1.md`). Missing/stale live depth snapshots remain mandatory skips.
- `endgame_sweep_v1`: fixed the dynamic DVOL entry path that could evaluate valid candidates but produce zero submits. The default Endgame entry floor is now the market minimum `0.01` instead of the old pre-DVOL `0.5`, share sizing now accepts the legacy desktop `EVPOLY_ENDGAME_BASE_SIZE_USD` profile field as a fallback when `EVPOLY_ENDGAME_BASE_SIZE_SHARES` is blank, and share-mode Endgame can submit a resting edge-safe LIMIT order when no visible ask exists under the dynamic limit instead of skipping solely for missing visible executable depth (`src/config.rs`, `src/endgame_sweep.rs`, `src/main.rs`, `.env.full.example`, `docs/endgame_sweep_v1.md`). DVOL, CEX-depth, fee-aware edge, proxy freshness, and close-guard gates remain mandatory.
- `endgame_sweep_v1`: fixed live exact-anchor availability after Endgame fail-closed on current-period base anchors. Coinbase intraday REST anchors now compose `5m/15m/1h/4h` periods from exact 1-minute candle rows, force UTC `Z` query timestamps instead of Chrono's `+00:00` form that Coinbase returns empty for, and cache-bust retry requests so early empty current-period responses do not poison later retries; `HYPE` intraday REST anchors now use Binance futures 1-minute klines instead of Binance spot (`src/api.rs`, `docs/endgame_sweep_v1.md`). Exact timestamp matching and fail-closed behavior are unchanged.
- `endgame_sweep_v1`: hardened the dynamic Endgame path after a bad ETH 5m fill by requiring exact REST period-open anchors before any Endgame tick can trade, accepting only Coinbase/Binance candles whose timestamps equal the market open, using exact Coinbase plus Binance opens for BTC/ETH/SOL/XRP dual-proxy agreement, using exact Binance opens for Binance-routed symbols, bounding anchor fetches to 750ms with fail-closed retry, refreshing scheduler time after awaited anchor fetches, evaluating both UP and DOWN candidates through quote/CEX-depth/DVOL/fee/edge gates before selection, removing the old plan-level thin-flip whole-tick veto, requiring DVOL pass to include edge at the probed Polymarket price, and restoring a 250ms final submit guard in both strategy and trader/worker submit paths (`src/main.rs`, `src/api.rs`, `src/endgame_dvol.rs`, `src/trader.rs`). Endgame now fail-closes on missing open anchors instead of substituting a late live proxy snapshot, and queued submits are rejected if they reach the trader too close to market close.
- `endgame_sweep_v1`: changed the live Endgame entry path from a fixed 99c limit/backfilled-depth model to a dynamic fair-price limit model (`src/main.rs`, `src/trader.rs`, `src/endgame_sweep.rs`, `src/endgame_dvol.rs`, `src/size_policy.rs`, `.env.example`, `.env.full.example`, `docs/endgame_sweep_v1.md`). Each checkpoint from `t-10s` through `t-100ms` now has an independent hardcoded share allocation curve of `5/5.2/5.5/6/6.7/7.6/8.8/10.2/11.8/13.2/20` percent, summing to one full period allocation. CEX-depth and DVOL are mandatory readiness gates, the order limit is derived from DVOL fair probability minus Polymarket fees and the edge floor, sizing only uses visible asks under that dynamic limit, all due unprocessed checkpoints are evaluated urgent-first in one scheduler pass, the fast submit decision-age deadline guard was removed, and submitted worker requests reserve period budget immediately until they clearly fail.
- `endgame_sweep_v1`: ported the SaaS Book99 DVOL fair-price source into Endgame and expanded the default checkpoint schedule to `t-10s` through `t-1s` plus final `t-100ms` (`src/endgame_dvol.rs`, `src/main.rs`, `src/config.rs`, `src/bin/alpha_service.rs`, `src/size_policy.rs`, `.env.example`, `.env.full.example`, `docs/endgame_sweep_v1.md`). Endgame now refreshes Deribit BTC/ETH DVOL, applies Book99 ETH-DVOL synthetic multipliers plus RV30 overrides for alt symbols, converts current CEX distance-to-base and remaining tau into a DVOL fair probability, compares that fair against live Polymarket quote/VWAP plus fee and existing edge floor, keeps CEX-depth as a live required-distance/size input for every checkpoint, and removes the old pre-DVOL near-base / Polymarket mid-band vetoes from Endgame.
- `endgame_sweep_v1`: share-size Endgame entries now calculate expected VWAP from live ask levels under the 99c limit instead of assuming the full order fills at 99c, so a 92c visible offer is evaluated as a 92c opportunity for the DVOL fair-price gate (`src/main.rs`).
...
- `mm_sport_v1`: made Polymarket website rewards API discovery resilient to bad later cursor pages by keeping successful pages, skipping numeric cursor gaps when possible, and only falling back to CLOB current rewards when the first page fails; the request also now forces HTTP/1.1 with an explicit JSON `Accept` header (`src/api.rs`).
...
- Runtime wallet/order path: Safe Wallet BUY readiness now uses the cached CLOB pUSD balance/allowance path again (`src/api.rs`).
...
- `mm_sport_v1` / MM 2.0 fresh-entry safety hardening: fresh BUY quote discovery and the live quote loop now use an explicit reward-rate gate and, when `EVPOLY_MM_SPORT_REQUIRE_REWARD_ELIGIBLE=true`, require real reward metadata (`reward_min_size_shares > 0` and `reward_max_spread > 0`) before a market can be used for new entries (`src/main.rs`, `src/mm/mod.rs`, `.env.example`, `.env.full.example`, `docs/mm_sport_v1.md`).
...
- `evsnipe_v1`: confirmed-hit conditions now block later pre-hit legs for the same condition, and `EVPOLY_EVSNIPE_STRATEGY_CAP_USD` is honored as the runtime EVSnipe in-loop submit cap (`src/main.rs`, `src/evsnipe.rs`, `.env.example`, `.env.full.example`).
  - a fired pre-hit leg still allows the confirm leg to submit; only a completed confirm leg closes the condition for future EVSnipe hits.
...
- `mm_sport_v1` timeout fallback wake cadence is now hardcoded to `1000ms` with no env override (`src/mm/mod.rs`, `.env.example`, `.env.full.example`):
  - removed runtime env path `EVPOLY_MM_SPORT_EVENT_FALLBACK_POLL_MS`.
  - event-driven MM Sport still prefers websocket updates first, but its backup poll now always wakes once per second.
