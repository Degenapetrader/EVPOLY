...
use polymarket_arbitrage_bot::models::{Market, MarketDetails, OrderBook, OrderRequest, TokenPrice};
...
use polymarket_arbitrage_bot::ui_contracts::{
    ...
    UiMarketOrderbook, UiOrderbookLevel,
};
...
async fn resolve_ui_market_lookup(
    state: &AppState,
    target: &str,
) -> Result<(Market, Option<MarketDetails>), anyhow::Error> {
    let trimmed = target.trim();
    if trimmed.is_empty() {
        anyhow::bail!("market target missing");
    }

    if let Ok(resolved) =
        resolve_manual_market_request(state.api.as_ref(), Some(trimmed), None).await
    {
        return Ok(resolved);
    }

    resolve_manual_market_request(state.api.as_ref(), None, Some(trimmed)).await
}

fn build_ui_orderbook_levels(entries: &[polymarket_arbitrage_bot::models::OrderBookEntry], depth: usize) -> Vec<UiOrderbookLevel> {
    let mut running_total = 0.0_f64;
    entries
        .iter()
        .take(depth)
        .filter_map(|entry| {
            let price = entry.price.to_f64()?;
            let shares = entry.size.to_f64()?;
            running_total += shares;
            Some(UiOrderbookLevel {
                price,
                shares,
                total: running_total,
            })
...
fn build_ui_market_orderbook(
    side: &polymarket_arbitrage_bot::ui_contracts::UiMarketSide,
    book: Option<OrderBook>,
    depth: usize,
) -> UiMarketOrderbook {
    if let Some(book) = book {
        let bids = build_ui_orderbook_levels(book.bids.as_slice(), depth);
        let asks = build_ui_orderbook_levels(book.asks.as_slice(), depth);
        ...
        return UiMarketOrderbook {
            token_id: side.token_id.clone(),
            label: side.label.clone(),
            outcome: side.outcome.clone(),
...
async fn handle_ui_market_detail(
    State(state): State<AppState>,
    headers: HeaderMap,
    AxPath(target): AxPath<String>,
) -> Result<Json<Value>, ApiError> {
    check_auth(&headers, &state)?;
    let (market, details) = resolve_ui_market_lookup(&state, target.as_str())
        .await
        .map_err(market_lookup_error)?;
    Ok(Json(json!({
        "ok": true,
...
async fn handle_ui_market_orderbook(
    State(state): State<AppState>,
    headers: HeaderMap,
    AxPath(target): AxPath<String>,
    ...
    check_auth(&headers, &state)?;
    let depth = query.depth.unwrap_or(6).clamp(1, 12);
    let (market, details) = resolve_ui_market_lookup(&state, target.as_str())
        .await
        .map_err(market_lookup_error)?;
    let ui_market = build_ui_market(&market, details.as_ref());
    let mut books = Vec::with_capacity(ui_market.sides.len());

    for side in ui_market.sides.iter() {
        let book = match state.api.get_orderbook(side.token_id.as_str()).await {
            Ok(book) => Some(book),
            Err(error) => {
                let message = error.to_string().to_ascii_lowercase();
                if message.contains("no_orderbook") || message.contains("no orderbook exists") {
                    None
                } else {
                    return Err(ApiError::new(
                        StatusCode::BAD_GATEWAY,
                        format!("orderbook lookup failed for {}: {}", side.label, error),
                    ));
                }
            }
        };
        books.push(build_ui_market_orderbook(side, book, depth));
    }
...
async fn main() -> Result<()> {
    ...
    let app = Router::new()
        ...
        .route("/ui/market/{target}/detail", get(handle_ui_market_detail))
        .route("/ui/market/{target}/orderbook", get(handle_ui_market_orderbook))
