...
async fn main() -> Result<()> {
    ...
    if args.list || args.sell_all || args.token_id.is_none() {
        ...
        let tokens_result: Result<Vec<(String, f64, String)>, _> = api
            .get_portfolio_tokens(btc_condition_id, eth_condition_id)
            .await;
        match tokens_result {
            Ok(tokens) => {
                if tokens.is_empty() {
                ...
                for (idx, (token_id, balance, description)) in tokens.iter().enumerate() {
                    ...
                    if let Ok(Some(price)) = api.get_best_price(token_id).await {
                        if let Some(bid) = price.bid {
                            ...
                            println!(
                                "      Estimated value: ${:.6}",
                                f64::try_from(bid).unwrap_or(0.0) * balance
                            );
                        }
                    }
                    println!();
                ...
                if args.token_id.is_none() && !tokens.is_empty() {
                    println!(
                        "💰 No token ID specified. Using first token: {}\n",
                        tokens[0].2
                    );
...
async fn sell_token(
    ...
    match api.check_balance_allowance(token_id).await {
        Ok((balance_decimal, allowance_decimal)) => {
            ...
            if shares_to_sell > balance_f64 {
                println!(
                    "   ⚠️  Requested shares ({:.6}) > balance ({:.6}), selling all available",
            ...
            if allowance_f64 < shares_to_sell {
                println!(
                    "   ⚠️  WARNING: Token allowance ({:.6}) is less than shares to sell ({:.6})",
                ...
                println!(
                    "   🔄 The SDK should handle approval automatically when placing the order."
                );
                println!(
                    "   💡 If the order fails, you may need to manually approve on Polymarket UI."
                );
        ...
        Err(e) => {
            eprintln!(
                "   ⚠️  Failed to check balance/allowance: {} - proceeding anyway",
                e
            );
    ...
    println!(
        "   Expected revenue: ${:.6}",
        current_price * shares_to_sell
    );

    // Attempt to sell
    match api
        .place_market_order(token_id, shares_to_sell, "SELL", Some("FAK"))
        .await
    {
        Ok(response) => {
